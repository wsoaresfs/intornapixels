...
let externalState={jobId:null,target:null,pack:null};
...
async function invokeExternal(body){
  let r=await db.functions.invoke('production-external',{body});
  if(r.error?.context?.status===401){const x=await db.auth.refreshSession();if(!x.error)r=await db.functions.invoke('production-external',{body})}
  if(r.error){let msg=r.error.message;try{const c=r.error.context;if(c?.clone){const j=await c.clone().json();msg=j?.error||msg}}catch{}throw new Error(msg)}
  if(r.data?.error)throw new Error(String(r.data.error));return r.data||{};
}
...
function addStyles(){
 ...
 .ip20-btn.primary{background:#2563eb;color:white}.ip20-btn.green{background:#16a34a;color:white}.ip20-btn.gold{background:#f59e0b;color:#111827}.ip20-btn.ghost{background:#eef2f7;color:#334155}.ip20-btn.danger{background:#fee2e2;color:#991b1b}.ip20-btn.dark{background:#111827;color:white}.ip20-btn.chatgpt{background:#111827;color:#fff}.ip20-btn.flow{background:#6d28d9;color:#fff}
 ...
 .ip20-fallback{margin-top:9px;padding:9px;border:1px solid #fde68a;background:#fffbeb;border-radius:10px;font-size:10px;line-height:1.45;color:#92400e}
 .ip20-modal{position:fixed;inset:0;z-index:99999;background:rgba(15,23,42,.62);display:grid;place-items:center;padding:18px}.ip20-modal[hidden]{display:none!important}.ip20-modalcard{width:min(760px,96vw);max-height:90vh;overflow:auto;background:#fff;border-radius:18px;padding:18px;box-shadow:0 25px 80px rgba(15,23,42,.3)}
 .ip20-refgrid{display:flex;gap:7px;flex-wrap:wrap;margin:10px 0}.ip20-note{font-size:11px;line-height:1.55;color:#475569}.ip20-kitlist{margin:8px 0 12px;padding-left:19px;font-size:11px;line-height:1.6}
...
function bind(){
 ...
 $('#ip20ExternalClose').onclick=closeExternal;$('#ip20ExternalCopy').onclick=copyExternalPrompt;$('#ip20ExternalOpen').onclick=openExternalProvider;$('#ip20ExternalKit').onclick=downloadExternalKit;$('#ip20ExternalImport').onclick=()=>$('#ip20ExternalFiles').click();$('#ip20ExternalFiles').onchange=e=>importExternalResults(e.target.files);
}
...
function renderJobs(){
 ...
 host.innerHTML=rows.map(j=>{
  ...
  const hybrid=['ready','failed'].includes(String(j.status)),quotaHint=/quota|billing|cota|limite|rate-limit/i.test(String(j.error_message||''));
  return `<div class="ip20-row"><div class="ip20-rowtop"><div><span class="ip20-status ${statusClass(j.status)}">${safe(statusLabel(j.status))}</span><h3 style="margin:7px 0 2px">${safe(one(o.clients)?.name||'Cliente')} • ${safe(o.essay_type||'Ensaio')}</h3><small>${safe(j.mode)} • ${done}/${total} imagens • ${safe(j.provider)} • ${safe(j.quality)}</small></div><b>${pct}%</b></div><div class="ip20-progress"><span style="width:${pct}%"></span></div>${j.error_message?`<p style="color:#b91c1c">${safe(j.error_message)}</p>`:''}${quotaHint?'<div class="ip20-fallback">A API está sem cota. O ensaio pode continuar sem perder o briefing: gere no ChatGPT ou no Google Flow e importe as imagens de volta.</div>':''}<div class="ip20-actions">${j.status==='ready'?`<button class="ip20-btn green" data-ip20-generate="${j.id}" ${busy?'disabled':''}>${busy?'Gerando…':'🚀 Gerar automaticamente'}</button>`:''}${hybrid?`<button class="ip20-btn chatgpt" data-ip20-chatgpt="${j.id}">🤖 Gerar no ChatGPT</button><button class="ip20-btn flow" data-ip20-flow="${j.id}">🎬 Gerar no Flow</button><button class="ip20-btn ghost" data-ip20-import="${j.id}">⬆ Importar resultados</button>`:''}${['human_review','approved','published'].includes(j.status)?`<button class="ip20-btn primary" data-ip20-review="${j.id}">✅ Revisar imagens</button>`:''}<button class="ip20-btn ghost" data-ip20-prompt="${j.id}">Copiar prompt</button>${!['published','completed','cancelled'].includes(j.status)?`<button class="ip20-btn danger" data-ip20-cancel="${j.id}">Cancelar</button>`:''}</div></div>`;
 }).join('');
 $$('[data-ip20-generate]',host).forEach(b=>b.onclick=()=>generateLoop(b.dataset.ip20Generate,b));
 $$('[data-ip20-chatgpt]',host).forEach(b=>b.onclick=()=>openExternal(b.dataset.ip20Chatgpt,'chatgpt'));
 $$('[data-ip20-flow]',host).forEach(b=>b.onclick=()=>openExternal(b.dataset.ip20Flow,'flow'));
 $$('[data-ip20-import]',host).forEach(b=>b.onclick=()=>startImport(b.dataset.ip20Import));
...
function closeExternal(){const m=$('#ip20ExternalModal');if(m)m.hidden=true}
function copyExternalPrompt(){const p=externalState.pack?.prompt||'';if(!p)return;navigator.clipboard.writeText(p).then(()=>toast('Prompt do Diretor IA copiado.')).catch(()=>toast('Não foi possível copiar o prompt.'))}
function openExternalProvider(){const u=externalState.pack?.providerUrl;if(!u)return;window.open(u,'_blank','noopener,noreferrer')}
function downloadExternalKit(){
 const p=externalState.pack;if(!p)return;const lines=[`INTORNÁ PIXELS • KIT ${p.label||''}`,`Cliente: ${p.clientName||'Cliente'}`,`Ensaio: ${p.essayType||'Ensaio'}`,`Pacote: ${p.packageName||''}`,`Quantidade restante: ${p.remaining??p.quantity??0}`,`Proporção: ${p.aspectRatio||''}`,`Qualidade: ${p.quality||''}`,'','PASSOS',...(p.instructions||[]).map((x,i)=>`${i+1}. ${x}`),'','REFERÊNCIAS',...(p.references||[]).map((r,i)=>`${i+1}. ${r.url}`),'','PROMPT DO DIRETOR IA',p.prompt||''];
 const blob=new Blob([lines.join('\n')],{type:'text/plain;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`INTORNA_${String(p.clientName||'ENSAIO').replace(/[^a-z0-9]+/gi,'_')}_${String(p.target||'EXTERNO').toUpperCase()}.txt`;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},500);
}
async function openExternal(jobId,target){
 const m=$('#ip20ExternalModal');m.hidden=false;$('#ip20ExternalTitle').textContent='Preparando kit…';$('#ip20ExternalSummary').textContent='Gerando links seguros das referências.';$('#ip20ExternalRefs').innerHTML='';$('#ip20ExternalSteps').innerHTML='';$('#ip20ExternalPrompt').textContent='';
 try{const p=await invokeExternal({action:'package',jobId,target});externalState={jobId,target,pack:p};$('#ip20ExternalTitle').textContent=`${target==='flow'?'🎬':'🤖'} Gerar no ${p.label}`;$('#ip20ExternalSummary').textContent=`${p.clientName||'Cliente'} • ${p.remaining} imagem(ns) restante(s) • proporção ${p.aspectRatio||'—'} • referências válidas por 30 minutos.`;$('#ip20ExternalSteps').innerHTML=(p.instructions||[]).map(x=>`<li>${safe(x)}</li>`).join('');$('#ip20ExternalRefs').innerHTML=(p.references||[]).length?(p.references||[]).map((r,i)=>`<a class="ip20-btn ghost" target="_blank" rel="noopener noreferrer" href="${safe(r.url)}">📷 Abrir referência ${i+1}</a>`).join(''):'<span class="ip20-note">Nenhuma referência disponível.</span>';$('#ip20ExternalPrompt').textContent=p.prompt||'';$('#ip20ExternalOpen').textContent=`Abrir ${p.label}`;navigator.clipboard.writeText(p.prompt||'').then(()=>toast(`Kit do ${p.label} pronto. Prompt copiado.`)).catch(()=>toast(`Kit do ${p.label} pronto.`));}
 catch(e){closeExternal();toast(e.message||'Falha ao preparar o kit externo.')}
}
function startImport(jobId){const j=state.jobs.find(x=>x.id===jobId);externalState={jobId,target:'manual',pack:{jobId,quantity:Number(j?.quantity||0),remaining:Math.max(0,Number(j?.quantity||0)-Number(j?.generated_count||0))}};$('#ip20ExternalFiles').value='';$('#ip20ExternalFiles').click()}
async function importExternalResults(fileList){
 const input=$('#ip20ExternalFiles'),all=[...(fileList||[])];if(!all.length)return;const jobId=externalState.jobId;if(!jobId){toast('Abra um job antes de importar.');return}
 const j=state.jobs.find(x=>x.id===jobId),quantity=Number(j?.quantity||externalState.pack?.quantity||0),remaining=Math.max(0,quantity-Number(j?.generated_count||0));if(remaining<=0){toast('Esse ensaio já atingiu a quantidade planejada.');input.value='';return}
 const files=all.slice(0,Math.min(remaining,20));if(all.length>files.length)toast(`Serão importadas somente ${files.length} imagem(ns), que é o restante planejado.`);
 try{toast(`Preparando ${files.length} imagem(ns) para importação…`);const prep=await invokeExternal({action:'prepare_upload',jobId,files:files.map(f=>({name:f.name,type:f.type,size:f.size}))});const uploaded=[];for(let i=0;i<(prep.uploads||[]).length;i++){const u=prep.uploads[i],f=files[i];const r=await db.storage.from('intorna-media').uploadToSignedUrl(u.path,u.token,f,{contentType:f.type});if(r.error)throw r.error;uploaded.push({path:u.path,type:f.type,size:f.size})}const d=await invokeExternal({action:'register_uploads',jobId,target:externalState.target||'manual',items:uploaded});toast(d.done?`${d.inserted||0} imagem(ns) importadas. Ensaio pronto para revisão.`:`${d.inserted||0} imagem(ns) importadas. Restam ${d.remaining||0}.`);closeExternal();await loadAll(false);if(d.done)await openReview(jobId)}catch(e){toast(e.message||'Falha ao importar resultados.')}finally{input.value=''}
}

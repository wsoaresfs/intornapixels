...
<div class="grid">
 ...
 <div class="card two"><div class="rc20-head"><div><h2>🔐 Segurança RC21</h2><p class="muted">Master explícito, Launcher com segredo local, ações limitadas e trilha de auditoria.</p></div></div><div class="rc20-note">O Launcher <b>não aceita comandos livres</b>. Ele executa somente: status, iniciar pilha, renovar túnel, reiniciar WAHA e diagnóstico. O segredo do agente permanece no notebook e o backend armazena apenas o hash.</div><div class="rc20-actions"><button class="btn gold" id="rc20OpenMyStudio2">📸 Abrir Meu Estúdio</button></div></div>
</div>`}
...
function renderWaha(){
 ...
 $('#rc20TunnelInfo').innerHTML=/trycloudflare\.com/i.test(base)?'🟡 <b>Quick Tunnel:</b> modo demonstração R$0. O RC21 Launcher pode renovar a URL automaticamente.':'🟢 Endpoint HTTPS persistente/configurado.';
}
...
async function loadAll(show=false){if(state.busy)return;state.busy=true;try{await Promise.allSettled([loadWaha(),loadLauncher()]);if(show)toast('Diagnóstico RC21 atualizado.')}finally{state.busy=false}}

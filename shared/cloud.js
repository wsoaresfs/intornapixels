(function(global){
  ...
  async function bootstrap(profile={}){const {data,error}=await client.functions.invoke('bootstrap-account',{body:profile});if(error)throw error;if(data?.error)throw new Error(data.error);return data}
  ...
  function normalizeWorkspace(ws){
    ...
    if(orderMap.size){
      const next={};Object.entries(ws.checklists).forEach(([k,v])=>next[orderMap.get(k)||k]=v);ws.checklists=next;
  ...
  async function fetchWorkspace(ctx){
    ...
    return normalizeWorkspace({
      ...
      clients:(clientsR.data||[]).map(c=>({id:c.id,name:c.name,email:c.email||'',whatsapp:c.whatsapp||'',city:c.city||'',importantDate:c.important_date||'',notes:c.notes||'',createdAt:c.created_at})),
  ...
  async function syncWorkspace(ctx,workspace){
    ...
    try{
      ...
      const clientRows=ws.clients.map(c=>({id:c.id,studio_id:sid,name:c.name,email:c.email||null,whatsapp:c.whatsapp||null,city:c.city||null,important_date:c.importantDate||null,notes:c.notes||null,created_at:c.createdAt||new Date().toISOString()}));
  ...
  async function hydrateWorkspace(ctx){
    ...
    let legacy=null;try{legacy=JSON.parse(localStorage.getItem(KEY)||'null')}catch{}
    const remoteEmpty=!remote.clients.length&&!remote.orders.length;
    if(remoteEmpty&&legacy&&((legacy.clients||[]).length||(legacy.orders||[]).length)){
      const migrated=normalizeWorkspace(clone(legacy));migrated.config={...defaults,...migrated.config,brand:ctx.studio.name,plan:ctx.studio.plan_id};
      await syncWorkspace(ctx,migrated);return migrated;
    }
    return remote;
  }

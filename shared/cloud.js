...
const LAST_STUDIO_KEY='intorna_last_studio_rc20';

const defaults={
  brand:'Meu Estúdio',whatsapp:'',pix:'',deadline:'24 horas',
  delivery:'Olá, {cliente}! Seu ensaio está pronto. Obrigado por confiar no nosso trabalho.',
  plan:'free',watermarkText:'PRÉVIA • {marca}',watermarkOpacity:32,
  watermarkPosition:'diagonal',watermarkSize:10,extraPhotoPrice:4.90,
  bundle3Price:12.90,bundle5Price:19.90
};

const statusFromDb={
  aguardando_fotos:'Aguardando fotos',pagamento_pendente:'Pagamento pendente',
  em_producao:'Em produção',em_revisao:'Em revisão',pronto:'Pronto para entrega',
  entregue:'Entregue',cancelado:'Cancelado'
};
const payFromDb={
  pending:'Pendente',confirmed:'Parcial',received:'Pago',
  overdue:'Pendente',refunded:'Pendente',cancelled:'Pendente',failed:'Pendente'
};
...
async function invokeFunction(name,body={}){
  let {data,error}=await client.functions.invoke(name,{body});
...
function roleRank(role){return ({owner:0,admin:1,operator:2})[String(role||'')]??9}
async function getMembership(userId){
  ...
  rows.sort((a,b)=>{
    const ao=a.studios?.owner_user_id===userId?0:1,bo=b.studios?.owner_user_id===userId?0:1;
...
async function context(profile={}){
  ...
  let boot=null;try{boot=await bootstrap(profile)}catch(e){console.warn('RC20 bootstrap',e)}
  const admin=boot?.is_platform_admin??await isAdmin(user.id);
  let member=await getMembership(user.id);
  if(!member&&boot?.studio_id){
    const {data:studio,error}=await client.from('studios')
      ...
      .eq('id',boot.studio_id).maybeSingle();
...
async function route(profile={}){
  const ctx=await context(profile);if(!ctx)return '/portal/';
...
async function requireStudio(){
  ...
  if(impersonate){
    ...
    if(member){ctx.member=member;ctx.studioId=member.studio_id;ctx.studio=member.studios;ctx.impersonating=false}
  }
  ...
  if(['blocked','cancelled'].includes(String(ctx.studio.status))&&!ctx.isAdmin){
    await client.auth.signOut();alert('Esta conta não está liberada. Entre em contato com o suporte.');
    location.replace('/portal/');return null;
  }

  // RC20: nunca reaproveita cache operacional de outro estúdio.
  const previous=localStorage.getItem(LAST_STUDIO_KEY);
  if(previous&&previous!==ctx.studioId)localStorage.removeItem(KEY);
  localStorage.setItem(LAST_STUDIO_KEY,ctx.studioId);
  return ctx;
}
async function requireAdmin(){
  const ctx=await context();if(!ctx){location.replace('/portal/');return null}
  if(!ctx.isAdmin){location.replace('/app/');return null}
  return ctx;
}
async function signOut(){
  await client.auth.signOut();
  localStorage.removeItem(KEY);localStorage.removeItem(LAST_STUDIO_KEY);
  localStorage.removeItem('intorna_impersonate_studio');location.href='/portal/';
}
function normalizeWorkspace(ws){
  ...
  if(clientMap.size){
    const crm={};Object.entries(ws.config.crm||{}).forEach(([k,v])=>crm[clientMap.get(k)||k]=v);ws.config.crm=crm;
    if(Array.isArray(ws.config.quotes))ws.config.quotes.forEach(q=>{if(clientMap.has(q.clientId))q.clientId=clientMap.get(q.clientId)});
  }
  if(orderMap.size){
    const checks={};Object.entries(ws.checklists).forEach(([k,v])=>checks[orderMap.get(k)||k]=v);ws.checklists=checks;
    const paid={};Object.entries(ws.config.orderPaymentDates||{}).forEach(([k,v])=>paid[orderMap.get(k)||k]=v);ws.config.orderPaymentDates=paid;
    if(Array.isArray(ws.config.quotes))ws.config.quotes.forEach(q=>{if(orderMap.has(q.orderId))q.orderId=orderMap.get(q.orderId)});
  }
  return ws;
}
async function fetchWorkspace(ctx){
  ...
  (salesR.data||[]).forEach(s=>(salesBy[s.order_id]??=[]).push({
    id:s.id,qty:Number(s.quantity||0),value:Number(s.amount||0),
    payment:s.payment_status==='received'?'Pago':'Pendente',createdAt:s.created_at
  }));
  ...
  return normalizeWorkspace({
    ...
    clients:(clientsR.data||[]).map(c=>({
      id:c.id,name:c.name,email:c.email||'',whatsapp:c.whatsapp||'',city:c.city||'',
      importantDate:c.important_date||'',notes:c.notes||'',whatsappStage:c.whatsapp_stage||'novo_lead',
      lastContactAt:c.last_contact_at||null,lastContactNote:c.last_contact_note||'',createdAt:c.created_at
    })),
    orders:(ordersR.data||[]).map(o=>({
      id:o.id,clientId:o.client_id,packageId:o.package_id||String(o.package_name||'essencial').toLowerCase(),
      type:o.essay_type||'',format:o.format||'4:5',payment:payFromDb[o.payment_status]||'Pendente',
      paymentMethod:o.payment_method||'Pix',status:statusFromDb[o.status]||'Aguardando fotos',
      deadline:o.deadline_at||'',extras:Array.isArray(o.extras)?o.extras:[],notes:o.notes||'',
      extraOfferQty:Number(o.extra_offer_qty||0),extraStrategy:o.extra_strategy||'avulsa',
      watermarkExtras:o.watermark_extras!==false,extraSales:salesBy[o.id]||[],total:Number(o.order_value||0),
      productionStatus:o.production_status||'draft',productionPriority:Number(o.production_priority||50),
      generationMode:o.generation_mode||'assisted',createdAt:o.created_at
    })),
    checklists,createdAt:studio.created_at
  });
}
async function hydrateWorkspace(ctx){
  // RC20: Supabase é a fonte de verdade. O cache local NUNCA migra automaticamente para um estúdio vazio.
  const remote=await fetchWorkspace(ctx);
  try{localStorage.setItem(`intorna_snapshot_${ctx.user.id}_${ctx.studioId}`,JSON.stringify(remote))}catch{}
  return remote;
}
async function fallbackSync(){throw new Error('Camada segura de sincronização ainda não foi instalada.')}
fallbackSync.__rc20Fallback=true;
function fallbackInstall(){console.warn('RC20: sincronização delta será instalada pelo bootstrap-cloud.')}
fallbackInstall.__rc20Fallback=true;
...
global.IntornaCloud={
  client,cfg,KEY,defaults,getSession,getUser,bootstrap,isAdmin,getMembership,context,route,
  requireStudio,requireAdmin,signOut,normalizeWorkspace,fetchWorkspace,hydrateWorkspace,
  syncWorkspace:fallbackSync,installWorkspaceSync:fallbackInstall,listTickets,sendTicket,notices,
  invokeFunction,adminAction,createSubscription,listSubscriptions,listBillingPayments,cancelSubscription
};
})(window);
